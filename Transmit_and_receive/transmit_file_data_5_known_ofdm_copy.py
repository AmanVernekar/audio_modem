...
five_known_datachunk = np.tile(known_datachunk, (5, 1))
...
five_modulated_seq_data = np.tile(known_modulated_seq_data, 5)
modulated_sequence_with_known = np.concatenate((five_modulated_seq_data, modulated_sequence))
...
def create_ofdm_datachunks(modulated_sequence, chunk_length, lower_bin, upper_bin):
    ...
    if phase_insertion: 
        ...
        #     np.nan))))
        phases = np.angle(separated_mod_sequence) - np.pi/4
        ofdm_datachunk_sub = ofdm_datachunk_array[:, lower_bin:upper_bin+1]
        first, second = ofdm_datachunk_sub.shape
        ofdm_sub_rotated = np.zeros(ofdm_datachunk_sub.shape, dtype='complex')
        for i in range(first):
            for k in range(second):
                if phases[i][k] != 0:
                    ofdm_sub_rotated[i][k] = ofdm_datachunk_sub[i][k] * np.exp(1j * phases[i][k])
                else:
                    ofdm_sub_rotated[i][k] = ofdm_datachunk_sub[i][k]
        
        # print(f'{separated_mod_sequence[0,:10]}')
        # # print(f'{phases[0,:10]}')
        # print(f'{ofdm_datachunk_sub[0,:10]}')
        # print(f'{ofdm_sub_rotated[0,:10]}')
        # print(f'{(ofdm_sub_rotated/(ofdm_datachunk_sub/(1+1j)))[0,:10]}')
        # exit()
        ofdm_datachunk_array[:, lower_bin:upper_bin+1] = ofdm_sub_rotated  
        ofdm_datachunk_array[:, chunk_length-upper_bin:(chunk_length-lower_bin)+1] = np.fliplr(np.conjugate(ofdm_sub_rotated))     
    else: 
...
ofdm_datachunks = np.concatenate((five_known_datachunk, ofdm_datachunks), axis=0)
print(f"The shape of the ofdm_datachunk_array with known five added is {ofdm_datachunks.shape}")
...
output_file = f'Data_files/new_phase_example_file_audio_to_test_with.wav'
