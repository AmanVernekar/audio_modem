...
plt.title("Channel impulse before resynchronisation")
plt.xlabel('Index')
plt.ylabel('Absolute value of channel impulse')
plt.show()
...
# plt.xlabel('Index')
# plt.ylabel('Absolute value of channel impulse')
# plt.axvline(np.argmax(np.abs(channel_impulse)), color = 'red', label='maximum channel impulse')
# plt.legend()
# plt.show()

plt.plot(np.roll(np.abs(channel_impulse), 1000))
plt.title("Channel impulse with peaks at the end rolled forward")
plt.xlabel('Index')
plt.ylabel('Absolute value of channel impulse')
# plt.axvline(np.argmax(np.abs(channel_impulse)), color = 'red', label='maximum channel impulse')
plt.legend()
...
# plt.title("Channel impulse before resynchronisation")
# ten_p = np.where(channel_impulse)
# plt.show()
...
for g, shift in enumerate(shifts):
    ...
    detected_chirp = recording[detected_index-n+shift:detected_index+shift]
    detected_fft = fft(detected_chirp)
    channel_fft = detected_fft/chirp_fft
    channel_impulse = ifft(channel_fft)

    # plt.plot(np.roll(np.abs(channel_impulse), 800))
    # plt.title("Channel impulse with peaks at the end rolled forward")
    # plt.xlabel('Index')
    # plt.ylabel('Absolute value of channel impulse')
    # # plt.axvline(np.argmax(np.abs(channel_impulse)), color = 'red', label='maximum channel impulse')
